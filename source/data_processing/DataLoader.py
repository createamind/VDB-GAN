""" Module for the data loading pipeline of the model (training part) """
...
class FlatDirectoryImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            img = img.crop((0, 20, 178, 198)) # crop the original image (178,218) to (178,178)
            img = self.transform(img)         # resize the image to (128,128)

        # convert the black and white image to RGB:
        if img.shape[0] < 3:
            img = img.expand(3, -1, -1)

        # truncate the alpha channel of the image if one exists
        if img.shape[1] >= 4:
            img = img[:3, :, :]
